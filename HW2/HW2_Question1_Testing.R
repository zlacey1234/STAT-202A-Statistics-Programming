...
b1 = as.points(earthquakeLong, earthquakeLat)

kernelBoundary = matrix(c(-118.5, 33.5, 
                          -116.5, 33.5, 
                          -116.5, 34.5, 
                          -118.5, 34.5, 
                          -118.5, 33.5), 
...
zz = MASS::kde2d(earthquakeLong, earthquakeLat)

ix = findInterval(earthquakeLong, zz$x)
iy = findInterval(earthquakeLat, zz$y)
ii = cbind(ix, iy)
...
z = kernel2d(b1, kernelBoundary, bandwidth, nx = 100, ny = 100)
...
print(matrix(rep(x4,2),ncol=100,byrow=T))

print(c(-1:1))


earthquakeDataTibble = tibble(magnitude = earthquakeMag, 
                              longitude = earthquakeLong,
                              latitude = earthquakeLat)


LosAngelesCountyLongLat = 
  c(left = -118.5, bottom = 33.75, right = -116.5, top = 34.5)

LosAngelesCountyMap = 
  get_stamenmap(LosAngelesCountyLongLat, maptype = "toner-lines") %>% 
  ggmap() 

LosAngelesCountyMap + 
  geom_point(aes(x = longitude, y = latitude, colour = "Earthquakes"), 
             data = earthquakeDataTibble) 